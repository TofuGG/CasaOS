#!/usr/bin/bash
#
#           TofuGG CasaOS Installer Script v0.4.18#
#   GitHub: https://github.com/TofuGG/CasaOS
#   Issues: https://github.com/TofuGG/CasaOS/issues
#   Requires: bash, mv, rm, tr, grep, sed, curl/wget, tar, smartmontools, parted, ntfs-3g, net-tools
#
#   This script installs CasaOS to your system. (Unofficial fork of CasaOS)
#   Usage:
#
...
echo '
===============================================================================
                     TofuGG CasaOS Installer v0.4.18
                     Unofficial fork of CasaOS
                     https://github.com/TofuGG/CasaOS
===============================================================================
'
...
# CASA_UNINSTALL_URL: the uninstaller is an UNCHANGED upstream component, so
# this intentionally keeps pointing at the upstream get.casaos.io URL.
# shellcheck disable=SC2016
...
# UNOFFICIAL FORK WARNING + RISK ACKNOWLEDGMENT
Fork_Disclaimer() {
    Warn '
================================================================================
...
================================================================================
  ...
  This is the TofuGG community fork of CasaOS - it is NOT the official
  release and is NOT affiliated with or endorsed by the official
  CasaOS / IceWhaleTech team.

  This fork may behave differently, break badly, or even cause DATA LOSS.
  It is provided AS-IS with NO warranty and NO official support.
  Do NOT report problems to the official CasaOS project.

  By continuing you acknowledge and agree that the fork maintainer
  (TofuGG) takes NO responsibility or blame if anything breaks, fails,
  or is lost. USE AT YOUR OWN RISK.
================================================================================
'
    if [[ "${CASA_ACCEPT_RISK:-0}" == "1" ]]; then
        Warn "CASA_ACCEPT_RISK=1 detected - skipping confirmation (automated install)."
        return 0
    fi

    if ! read -r -p "Type 'yes' to acknowledge the above and continue (anything else aborts): " acknowledgment < /dev/tty; then
        Show 1 "No terminal available and CASA_ACCEPT_RISK is not set. Aborting. For automated installs set CASA_ACCEPT_RISK=1."
        exit 1
    fi

    if [[ "${acknowledgment,,}" != "yes" ]]; then
        Show 1 "Aborted. You did not acknowledge the risk."
        exit 1
    fi

    Show 0 "Risk acknowledged. Continuing installation."
}
...
Check_Arch() {
    ...
    *)
    ...
    CASA_PACKAGES=(
        "${CASA_DOWNLOAD_DOMAIN}TofuGG/CasaOS-Gateway/releases/download/v0.4.10/linux-${TARGET_ARCH}-casaos-gateway-v0.4.10.tar.gz"
...
"${CASA_DOWNLOAD_DOMAIN}TofuGG/CasaOS-UserService/releases/download/v0.4.10/linux-${TARGET_ARCH}-casaos-user-service-v0.4.10.tar.gz"
...
"${CASA_DOWNLOAD_DOMAIN}TofuGG/CasaOS-AppManagement/releases/download/v0.4.18/linux-${TARGET_ARCH}-casaos-app-management-v0.4.18.tar.gz"
"${CASA_DOWNLOAD_DOMAIN}TofuGG/CasaOS/releases/download/v0.4.18/linux-${TARGET_ARCH}-casaos-v0.4.18.tar.gz"
...
"${CASA_DOWNLOAD_DOMAIN}TofuGG/CasaOS-UI/releases/download/v0.4.7/linux-all-casaos-v0.4.7.tar.gz"
...
Welcome_Banner() {
    ...
    echo -e "${GREEN_LINE}"
    echo -e ""
    echo -e " ${aCOLOUR[2]}TofuGG CasaOS : https://github.com/TofuGG/CasaOS"
    echo -e " ${aCOLOUR[2]}Issues        : https://github.com/TofuGG/CasaOS/issues"
    echo -e " ${aCOLOUR[2]}Note          : Unofficial fork of CasaOS"
    echo -e ""
...
# Show the unofficial-fork warning and require explicit risk acknowledgment
Fork_Disclaimer
...
# Mark this system as running the TofuGG fork (used by the update script)
# to distinguish fork installs from the official release.
if [[ -e /etc/casaos/.tofugg-fork ]]; then
    Show 2 "Fork marker already present."
else
    ${sudo_cmd} touch /etc/casaos/.tofugg-fork || Show 3 "Failed to write fork marker (non-fatal)."
fi
